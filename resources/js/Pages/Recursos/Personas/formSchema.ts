...
const PERFILES_READONLY = ['', 'Aplicador', 'Técnico', 'Supervisor', 'Productor'] as const
export const TIPOS_ROPO = ['', 'Aplicador', 'Técnico']
const TIPOS_ROPO_READONLY = ['', 'Aplicador', 'Técnico'] as const
export const TIPOS_APLICADOR = ['', 'Básico', 'Cualificado', 'Fumigación', 'Piloto', 'Aplicación Fitosanitarios']
const TIPOS_APLICADOR_READONLY = ['', 'Básico', 'Cualificado', 'Fumigación', 'Piloto', 'Aplicación Fitosanitarios'] as const
...
export const formSchema = z.object({
        ...
        errorMap: (issue, _ctx) => {
            switch (issue.code) {
              ...
              case 'invalid_enum_value':
                return { message: `${FIELD_MSG} debe ser uno de los siguientes: ${TIPOS_ID_NAC.join(', ')}` };
              default:
    ...
    tel: z.string()
        ...
        .optional(),
    perfil: z.enum(PERFILES_READONLY, {
        errorMap: (issue, _ctx) => {
            switch (issue.code) {
                case 'invalid_type':
                    return { message: `${FIELD_MSG} debe ser solo texto` };
                case 'invalid_enum_value':
                    return { message: `${FIELD_MSG} debe ser uno de los siguientes: ${PERFILES.join(', ')}` };
                default:
    ...
    observaciones: z.string()
        ...
        .optional(),
    ropo: z.object({
        tipo: z.enum(TIPOS_ROPO_READONLY, {
            errorMap: (issue, _ctx) => {
                switch (issue.code) {
                    case 'invalid_type':
                        return { message: `${FIELD_MSG} debe ser solo texto` };
                    case 'invalid_enum_value':
                        return { message: `${FIELD_MSG} debe ser uno de los siguientes: ${TIPOS_ROPO.join(', ')}` };
                    default:
                        return { message: SHOULD_BE_VALID_MSG };
                }
            }
        ...
        tipo_aplicador: z.enum(TIPOS_APLICADOR_READONLY, {
            errorMap: (issue, _ctx) => {
                switch (issue.code) {
                    case 'invalid_type':
                        return { message: `${FIELD_MSG} debe ser solo texto` };
                    case 'invalid_enum_value':
                        return { message: `${FIELD_MSG} debe ser uno de los siguientes: ${TIPOS_APLICADOR.join(', ')}` };
                    default:
                        return { message: SHOULD_BE_VALID_MSG };
                }
            }
