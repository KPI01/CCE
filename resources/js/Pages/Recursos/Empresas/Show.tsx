...
export default function Show(props: Props) {
    ...
    return (
        <ShowLayout pageTitle="Empresa" title={`${props.data.nombre}`} isAdmin={props.isAdmin} updated_at={props.data.updated_at} recurso="empresa">
            <div className="flex flex-col items-start gap-8 pt-10 px-48">
                <div>
                    ...
                    <div className="flex items-start gap-x-12 my-12">
                        <ItemProperty title="Dirección" value={props.data.direccion} textarea />
                            ...
                            (
                                <>
                                    <AccordionItem value='personas'>
                                        <AccordionTrigger>
                                            <Title title={`Personas (${props.data.personas?.length})`} />
                                        </AccordionTrigger>
                                        <AccordionContent className="flex">
                                            {props.data.personas?.map(p => {
                                                return (
                                                    <Link key={p.id} href={route('recurso.persona.show', p.id)} className="w-fit">
