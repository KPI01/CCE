...
class DuskServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        ...
        Browser::macro("assertDisabledByName", function ($element, $name) {
            return $this->assertDisabled("{$element}[name={$name}]");
        });
        Browser::macro("assertSelectedByName", function ($name, $value) {
            return $this->assertSelected(
                "select[name={$name}]",
                $value
            );
        });
        Browser::macro("assertSelectMissingOptionByName", function (
            $name,
            array|string $value
        ) {
            if (is_array($value)) {
                return $this->assertSelectMissingOptions(
                    "select[name={$name}]",
                    $value
                );
            }
            return $this->assertSelectMissingOption(
                "select[name={$name}]",
                $value
            );
        });
        Browser::macro("assertSelectHasOptionByName", function (
            $name,
            array|string $value
        ) {
            if (is_array($value)) {
                return $this->assertSelectHasOptions(
                    "select[name={$name}]",
                    $value
                );
            }
            return $this->assertSelectHasOption(
                "select[name={$name}]",
                $value
            );
        });
