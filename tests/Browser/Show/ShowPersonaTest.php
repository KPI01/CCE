...
class ShowPersonaTest extends DuskTestCase
{
        ...
        $this->browse(function (Browser $browser) {
            $browser
                ...
                ->assertInputValue(
                    ...
                    date("d/m/Y", strtotime($this->p->ropo["caducidad"]))
                )
                ->assertSelectedByName(
                    "ropo.tipo_aplicador",
                    $this->p->ropo["tipo_aplicador"]
                );
        });
    }
    ...
    public function testRegreso(): void
    {
        $this->browse(function (Browser $browser) {
            $browser
                ...
                ->pause(750)
                ->assertPathIs("/cce/app/recurso/personas");
        });
    }
    ...
    public function testAcciones(): void
    {
        $this->browse(function (Browser $browser) {
            ...
            $browser
                ->assertPresent("#edit-" . $this->p->id)
                ->assertPresent("#delete-" . $this->p->id);

            $browser
                ->assertVisible("#edit-" . $this->p->id)
                ->assertVisible("#delete-" . $this->p->id);

            $browser
                ->click("#edit-" . $this->p->id)
                ->pause(2000)
                ->assertPathIs(
                    "/cce/app/recurso/personas/" . $this->p->id . "/edit"
                );

            $browser
                ->visit(new Show($this->p))
                ->click("button#delete-" . $this->p->id)
                ->pause(250)
